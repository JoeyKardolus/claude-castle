...
Use whatever the host project uses, Python or TypeScript, matching the app you are prototyping for. Match the project's existing tooling; don't add a new package manager or runtime just for the prototype.
...
2. **Keyboard shortcuts**, listed at the bottom: `[a] add user  [d] delete user  [t] tick clock  [q] quit`. Bold the key, dim the description, or vice-versa.
...
Add a script to the project's existing task runner. The user should run `uv run <path>` / `pnpm run <prototype-name>` / equivalent — never need to remember a path.
...
- **Don't wire it to the real database.** Use an in-memory store unless the question is specifically about persistence. Never the research DB.
