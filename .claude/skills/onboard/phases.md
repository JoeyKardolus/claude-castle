...
3. Spending protection, one sentence, no user action: "I watch your spending myself: the castle checks its own bill every hour and warns you past 30 euro a month." In the same breath, the one optional errand they can do while you build: "if you want written meeting summaries, you can already fetch the key at console.anthropic.com (Billing, add a card with a 5 euro monthly limit, then API Keys, Create); or skip it, everything else works without it." (The watchdog does this; Scaleway offers no API for its console budget alerts, so Claude cannot set those. Mention once that Scaleway's own extra alert email exists at https://console.scaleway.com/billing/alerts if they ever want a second net; optional, never blocking.)
